...
def _decode_image_universal(image_bytes, filename=""):
    ...
    EXIF: NÃO precisa correção manual — o cv2.imdecode (OpenCV >= 3.4) já aplica a
    orientação EXIF automaticamente ao decodificar JPEG. Uma correção manual adicional
    aqui causava DUPLA rotação, virando fotos corretas de lado (bug identificado e
    corrigido em 12/08/2026 após relato de cartão de simulado não sendo lido)."""
    import cv2
    import numpy as np
    ...
    # Caminho padrão JPG/PNG via cv2 — orientação EXIF já é aplicada automaticamente
...
async def escanear_universal_post(foto: UploadFile = File(...)):
    ...
    # Se não achou na foto crua, tenta com correção de perspectiva (12/08/2026)
    if not qr_data:
        qr_data, _ = _detectar_qr_com_perspectiva(img)
...
async def escanear_universal_lote(fotos: List[UploadFile] = File(...)):
    ...
    try:
        for filename, image_bytes in all_files:
            ...
            # Fallback com correção de perspectiva (12/08/2026)
            if not qr_data:
                qr_data, _ = _detectar_qr_com_perspectiva(img)
